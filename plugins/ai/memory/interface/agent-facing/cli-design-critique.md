...
- **A warning promoted upward does not survive; every fact lives at exactly one tier.** The draft promoted "expensive mistake" facts to root on the argument that the mistake happens before the agent reads deeper. Review: "Doesn't belong at root level." The corrected structure gets the same protection for free — because the branch read is *forced* (the schemas exist nowhere else), the async caveat at the branch is guaranteed to be seen before `deploy new` can be invoked correctly. Same tier discipline downward: the global contract is stated once at root, and a branch restatement is "Redundant." A focused `--flag -h` view is not another tier: it repeats the complete leaf contract beside one uncommon parameter's detail so the view is safe as a first read, while both renderings compose from one authored schema.
