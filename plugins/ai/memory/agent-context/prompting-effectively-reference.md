...
Make examples diverse — vary them so the model generalizes the principle instead of latching onto an incidental pattern they share.
...
### Long-context techniques (20k+ token inputs)

When the input is large, three techniques materially improve retrieval — this is the single home for long-context placement:

- **Longform data at the top.** Put long documents above the query, instructions, and examples. Queries-at-the-end can improve response quality by up to ~30% on complex multi-document inputs.
- **Structure documents with XML metadata.** Wrap each in `<document>` with `<source>` and `<document_content>` (and an `index`); the attributes give the model provenance it can cite:
...
Analyze the report. Identify strategic advantages and Q3 focus areas.
```

- **Ground responses in quotes.** For accuracy in long documents, have the model extract relevant quotes first (`<quotes>`), then reason from them (`<analysis>`). This cuts through the noise.
