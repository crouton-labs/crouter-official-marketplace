---
kind: reference
when-and-why-to-read: When you are authoring a prompt for an internal Claude/Opus tool (a skill, agent, mode, command, or subagent prompt) and the prompting-effectively skill directs you here, this reference should be read because it holds the Opus 4.7-specific tuning (effort, tool triggering, literal instruction-following, thinking, agentic patterns) the overview omits.
short-form: Opus 4.7-specific prompt-engineering reference — model behaviors, output control, tool use, thinking, agentic systems.
