---
kind: skill
when-and-why-to-read: When you are crafting Claude prompts for complex tasks — long documents, tool workflows, or code generation — this skill should be read because it gives the structure (roles, explicit instructions, positive framing, strategic organization) that improves results.
short-form: Structure Claude prompts for clarity — roles, explicit instructions, context, positive framing, organization.
...
**MANDATORY:** Before authoring any prompt for internal Claude tools (skills, agents, modes, commands, subagent prompts), read [prompting-effectively-opus-4-7.md](prompting-effectively-opus-4-7.md) — the Opus 4.7-specific behavior reference. It covers model-specific tuning (effort, tool triggering, literal instruction following, subagent spawning, thinking, agentic patterns) that this architecture overview does not.

For detailed patterns and examples of the architecture principles below, see [prompting-effectively-reference.md](prompting-effectively-reference.md).
...
- Tool usage policies (→ [tool-design](../interface/agent-facing/tool-design.md) covers designing the tools themselves)
...
→ For the orthogonal API-level question — what goes in the `system` message vs the `user` message of a single API call — see [system-vs-user-prompt](system-vs-user-prompt.md).
...
**When negative framing earns its place:** the model has an observed prior toward the wrong behavior with no prompt cause. "Avoid Inter" works because Opus genuinely defaults to Inter. "Be conservative" in a code review actively suppresses findings (see prompting-effectively-opus-4-7.md). Otherwise: trust the default.
...
→ For how repetition interacts with token budgets, placement effects, and caching in long prompts, see [context-management](context-management.md).
...
- Agents → [multi-agent-orchestration](../orchestration/multi-agent-orchestration.md) for designing systems of cooperating agents
