---
name: review-prompt
description: Reconstruct an agent's full assembled prompt from its scattered sources, resolve variables to realistic example values, annotate provenance, and put it on the user's screen for line-by-line comments. Use when asked to review an existing prompt.
type: runbook
keywords: [prompt-review, prompt-aggregation, assembled-prompt, variables, human-review, citations]
---
...
When the comments come back and you revise the actual prompt sources, → [prompting-effectively](../prompting-effectively/SKILL.md) for the changes themselves.
