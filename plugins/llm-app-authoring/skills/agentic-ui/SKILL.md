---
name: agentic-ui
description: Interface patterns for surfacing agent work to a human — surface choice (chat/canvas/ambient/dashboard), streaming, reasoning disclosure, tool-call and state legibility, review gates, checkpoints, steering, multi-agent views — for both web and terminal/TUI. Use when building or reviewing the UI an agent presents to a human.
type: playbook
keywords: [agentic-ui, generative-ui, streaming, tui, terminal, diff-review, checkpoint, tool-call, agent-state, alt-screen]
---

# Designing UI for agentic interaction

The screen — or the terminal — is where the human reads what the agent is doing and decides whether to trust it. This is the pattern layer: *what to render and when*. The judgment layer (trust, autonomy, friction, the first principles) is the sibling [agentic-ux](../agentic-ux/SKILL.md); build on it, don't restate it.
...
- **Review gates are non-negotiable.** Show diffs/plans before applying, with accept/reject (per-hunk where possible). Removing the gate breaks trust faster than model quality can compensate (Cursor's 2025 auto-apply regression and the user revolt is the proof point).
...
- **Legibility affordances.** Inline citations/provenance on factual claims; calibrated confidence on variable or high-stakes output; a one-line rationale after autonomous acts. (These render `ux` principles 4–5.)

## Web layer

Generative/dynamic UI (tool output → components, not text walls), inline citations with hover preview, suggestion chips for capability onboarding, editable plan/todo surfaces, mission-control for async/multi-agent, budget/token meters with tiered warnings. The de-facto open vocabulary is Vercel AI Elements (Reasoning, Tool, Task, Plan, Sources, Checkpoint, Queue, Artifact). Full catalog — 30 patterns, each problem → use → avoid → exemplar: [reference.md](reference.md).

## Terminal layer

- **The alt-screen debate (unresolved — the live one).** Inline/normal buffer keeps native scrollback, selection, and search but flickers under streaming; alt-screen kills flicker and gives a fixed composer but destroys those affordances. Recommendation: **conversational/coding agents → inline + a differential cell-diff renderer** (kill flicker *without* alt-screen); **dashboard-style → alt-screen acceptable**; if alt-screen, always ship escape hatches (dump-to-scrollback, native-selection bypass, env kill-switch) and make it opt-in/toggleable. Never force-migrate (Gemini CLI rolled it back in days).
- **Differential redraw, not full repaint.** A TTY has no scene graph; "clear + reprint per token" floods emulators and flickers. Ink/React-on-TTY is now an anti-pattern for sustained streaming; cell-diff renderers (Bubble Tea, Crush, Claude Code's rewrite, `pi`) are state of the art.
...
Full catalog — 20 patterns, each constraint → solution → exemplar — and the alt-screen decision matrix: [reference.md](reference.md).
