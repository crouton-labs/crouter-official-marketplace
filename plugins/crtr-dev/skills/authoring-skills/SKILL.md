...
That's the whole format. No build step, no registry — drop the dir under `plugins/<plugin>/skills/`, and `crtr skill list` picks it up.
...
```bash
# 1. Scaffold
crtr skill new crtr-dev:my-new-skill --description "Use when …"
...
# 3. Validate
crtr doctor                                        # parses frontmatter, checks name drift
...
- Link siblings: `see [reference.md](./reference.md)` for tables and long lists.
