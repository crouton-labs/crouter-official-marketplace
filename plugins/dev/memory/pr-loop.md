...
Close out each finding where it was raised, so its own thread carries the resolution and a later reader never re-opens a settled concern. Two signals together: a reply that quotes or names the specific finding and states the commit that fixes it (or the reason no change is right), and a reaction on the finding itself as the machine-scannable triaged flag — the reply carries the substance, the reaction lets a scan see nothing is unhandled. GitHub offers only eight reactions (`+1` `-1` `laugh` `hooray` `confused` `heart` `rocket` `eyes`) and no checkmark, so adopt `rocket` for a finding a commit fixes and `eyes` for one acknowledged-and-deferred or explained-away; set them with `gh api repos/{owner}/{repo}/issues/comments/{id}/reactions -f content=rocket` for a PR-level comment or `.../pulls/comments/{id}/reactions` for a diff-line comment. A diff-line review thread also gets resolved once answered — a PR-level or bot comment cannot be resolved and relies on the reply plus reaction instead. Never signal handled on a finding that is not: if a finding is real and unfixed, leave it open and raise the decision rather than reacting it closed.
...
After the merge and its target-branch run settle, fetch the remote target and update the local target branch—normally `main`—before ending the loop. Fast-forward it when possible; if it has local-only commits, rebase them onto the remote target without dropping or pushing them. Perform that history change in an isolated managed worktree when the shared checkout is in use, so the local branch contains the merged change without disrupting another agent's checkout.
