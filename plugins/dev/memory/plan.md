...
Hold scope to the specification and the implementation work it necessarily requires. Prefer the simplest complete implementation; speculative features, future extensibility, adjacent cleanup, and other merely plausible additions stay out. If something seems likely desirable but is not explicitly or implicitly required by the specification, ask the user whether to include it through `crtr human` before finishing the document (`crtr human send -h`), wait for their answer, and reflect that decision in the plan.
