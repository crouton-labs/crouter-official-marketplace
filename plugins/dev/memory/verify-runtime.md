---
kind: knowledge
when-and-why-to-read: When you are asked to verify a PR, confirm a fix, manually test a change, or validate local work before pushing, this knowledge should be read because integration failures that static checks and code inspection cannot see get caught before the change is declared working.
short-form: Verify a change by running the app and observing behavior at the surface a user meets — not by re-running tests, typechecks, or the function in isolation.
---
...
**Look for the repository's own run and verification tooling first — even if you already know how to build and run.** A repo that ships a verification or evidence-capture protocol expects you to drive the surface through it; drive it some other way and you produce a verdict the review pipeline cannot replay. Check the contribution guide, the task runner (`package.json` scripts, `Makefile`, `justfile`), any `scripts/` lifecycle CLI, and any agent-facing command directory the repo carries.

- **A verification harness matching your surface** → use it and follow its setup. Mismatched surface → skip it. A harness that fails on mechanics unrelated to the change is harness rot — ask the user whether to patch it; don't FAIL the change over it.
- **Run/launch tooling but no verification harness** → use its build and launch primitives as your handle.
- **Neither** → cold start from the README, `package.json`, or `Makefile`. Timebox roughly 15 minutes. Stuck → report BLOCKED with exactly where it stopped.
...
These aren't a checklist — pick the ones the diff points at. Stop when you've covered the obvious adjacents or hit something worth a ⚠️. A probe that finds nothing is still a step: "🔍 passed `--from ''` → clean `error: --from requires a value`, exit 2." That the author didn't test it is exactly why it's worth knowing it holds.
...
**Claim:** <what it's supposed to do — your read of the diff and/or the stated claim; note any mismatch>

**Method:** <how you got a handle — which harness or run tooling, or cold start; what you launched>

### Steps

Each step is one thing you did to the **running app** and what it showed. Build/install/checkout are setup, not steps. Test runs and typecheck don't belong here — they're CI's output.

1. ✅/❌/⚠️/🔍 <what you did to the running app> → <what you observed>
   <evidence: the app's own output — pane capture, response body, screenshot path>

🔍 marks a probe — a step off the claim's happy path, trying to break it. At least one. A Steps list that's all ✅ and no 🔍 is a happy-path replay: still PASS, but you stopped at the first half.

**Screenshot / sample:** <the one frame a reviewer looks at to see the feature — image path for GUI/TUI, code block for library/API; omit for build/types-only>

### Findings

<Things you noticed. Not just bugs — friction, surprises, anything a first-time user would trip on. "Took three tries to find the right flag." "Error message on typo was unhelpful." "Default seems odd for the common case." "Works, but slower than I expected." Lower the bar: if it made you pause, it goes here. But the pause has to be yours, from running the app — not from reading the PR page. A red CI check, a review comment, someone else's bot: visible to anyone already, and you relaying it isn't an observation. Claim/diff mismatch, pre-existing breakage, and env notes also belong.

Each probe gets a line here even when it held — "🔍 empty `--from` → clean error" tells the author what *was* covered, which they can't see from a bare PASS.

Lead with ⚠️ for lines worth interrupting the reviewer for — those get hoisted above the PR comment fold. Plain bullets are context they'll find if they expand. Empty is fine if nothing stuck out — but nothing sticking out is itself rare.>
```
